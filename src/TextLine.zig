...
pub fn draw(self: Self, app: root.App, x: u32, y: u32, width: u32, height: u32) !void {
    ...
    var dx: u32 = 0;
    while (iter.nextCodepoint()) |cp| : (dx += self.font.w) {
        const ons = self.font.drawChar(@intCast(u16, cp), scale);
        for (ons) |col, py| {
            for (col) |row, px| {
                if (!row) continue;
...
pub fn getWidth(self: Self, app: root.App) u32 {
    _ = app;
    return self.font.w * @intCast(u32, self.text.len) * scale;
}

pub fn getHeight(self: Self, app: root.App) u32 {
    _ = app;
    return self.font.h * scale;
}
